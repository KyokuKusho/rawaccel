...
namespace rawaccel {
	...
	struct accel_linear_lut : lut_base<accel_linear_lut> {
		...
		accel_linear_lut(const table_args& args) :
			range({
				args.start,
				args.stop,
				args.num_elements
		...
		accel_linear_lut(const accel_args& args) :
			accel_linear_lut(args.lut_args) {}
	};

	struct accel_binlog_lut : lut_base<accel_binlog_lut> {
		fp_rep_range range;
		double x_start;
		bool transfer = false;
		value_t data[capacity] = { 1 };
		...
		accel_binlog_lut(const table_args& args) :
			range({
		...
		accel_binlog_lut(const accel_args& args) :
			accel_binlog_lut(args.lut_args) {}
	};
